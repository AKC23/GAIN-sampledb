...
if (($handle = fopen($csvFile, "r")) !== FALSE) {
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        ...
        if ($vehicleID === false || $foodTypeID === false || $PSID === false || $countryID === false || $sourceVolume === false || $unitID === false || $cropToFoodConvertedValue === false || $startYear === false || $endYear === false || $accessedDate === false) {
        ...
        echo "VehicleID: $vehicleID, FoodTypeID: $foodTypeID, PSID: $PSID, Country_ID: $countryID, PeriodicalUnit: '$periodicalUnit', SourceVolume: $sourceVolume, UnitID: $unitID, CropToFoodConvertedValue: $cropToFoodConvertedValue, StartYear: $startYear, EndYear: $endYear, AccessedDate: '$accessedDate', Source: '$source', Link: '$link', ProcessToObtainData: '$processToObtainData'<br>";

        $sql = "INSERT INTO total_crop_import (VehicleID, FoodTypeID, PSID, Country_ID, PeriodicalUnit, SourceVolume, UnitID, CropToFoodConvertedValue, StartYear, EndYear, AccessedDate, Source, Link, ProcessToObtainData) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iiiisdiissssss", $vehicleID, $foodTypeID, $PSID, $countryID, $periodicalUnit, $sourceVolume, $unitID, $cropToFoodConvertedValue, $startYear, $endYear, $accessedDate, $source, $link, $processToObtainData);
    ...
    if ($result) {
        while ($row = $result->fetch_assoc()) {
            echo "ID: {$row['ImportID']}, VehicleID: {$row['VehicleID']}, FoodTypeID: {$row['FoodTypeID']}, PSID: {$row['PSID']}, Country_ID: {$row['Country_ID']}, PeriodicalUnit: {$row['PeriodicalUnit']}, SourceVolume: {$row['SourceVolume']}, UnitID: {$row['UnitID']}, CropToFoodConvertedValue: {$row['CropToFoodConvertedValue']}, StartYear: {$row['StartYear']}, EndYear: {$row['EndYear']}, AccessedDate: {$row['AccessedDate']}, Source: {$row['Source']}, Link: {$row['Link']}, ProcessToObtainData: {$row['ProcessToObtainData']}<br>";
        }
    }
